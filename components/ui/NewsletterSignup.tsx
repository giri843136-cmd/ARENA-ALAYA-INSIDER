    ...
    return (
      <div className={`p-6 bg-white rounded-2xl border border-[#E4DDD5] ${className}`}>
        ...
        <form onSubmit={handleSubmit} className="flex gap-2">
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="flex-1 px-3 py-2 text-sm border border-[#E4DDD5] rounded-lg focus:border-[#C5AA8A] outline-none" required aria-label="Email for newsletter" />
          <button type="submit" disabled={status === "loading"} className="px-4 py-2 bg-[#C5AA8A] text-white text-sm rounded-lg hover:bg-[#B89A7A] disabled:opacity-50 transition-colors">
  ...
  return (
    <form onSubmit={handleSubmit} className={`flex items-center gap-2 ${className}`}>
      <div className="relative flex-1">
        <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A8178]" />
        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" className="w-full pl-9 pr-3 py-2 text-sm border border-[#E4DDD5] rounded-lg focus:border-[#C5AA8A] outline-none" required aria-label="Email for newsletter" />
      </div>
      <button type="submit" disabled={status === "loading"} className="px-4 py-2 bg-[#C5AA8A] text-white text-sm rounded-lg hover:bg-[#B89A7A] disabled:opacity-50 transition-colors whitespace-nowrap">
        {status === "loading" ? <Loader2 size={16} className="animate-spin" /> : "Subscribe"}
      </button>
