...
function TwoFactorSection() {
  ...
  return (<div className="widget">
    <div className="flex items-center gap-2 mb-6"><Smartphone size={16} className="text-[var(--admin-accent)]" /><div className="widget-title">TWO-FACTOR AUTHENTICATION</div>{status?.enabled && <span className="ml-auto text-[10px] px-2 py-0.5 rounded-full bg-[#4ADE80]/10 text-[#4ADE80] border border-[#4ADE80]/20">ENABLED</span>}</div>
    <div className={`p-3 rounded-lg border text-sm mb-4 ${status?.enabled ? "bg-[#4ADE80]/5 border-[#4ADE80]/20" : "bg-[#FBBF24]/5 border-[#FBBF24]/20"}`}><div className="flex items-center gap-2">{status?.enabled ? <><Check size={14} className="text-[#4ADE80]" /><span className="text-[#4ADE80] text-xs">2FA is active</span></> : <><AlertTriangle size={14} className="text-[#FBBF24]" /><span className="text-[#FBBF24] text-xs">2FA not enabled</span></>}</div></div>
    {!status?.enabled && setupMode === "idle" && <button onClick={handleGenerate} disabled={submitting} className="btn-admin">{submitting ? <Loader2 size={14} className="animate-spin mr-2" /> : <Smartphone size={14} className="mr-2" />}Set up 2FA</button>}
    {setupMode === "verify" && qrCode && (<div className="space-y-4">
      <div className="flex items-start gap-6">                    <div className="w-40 h-40 bg-white rounded-xl p-2 relative">{ }
                      <Image src={qrCode} alt="QR code for authenticator app" fill className="object-contain" unoptimized /></div><div className="text-sm space-y-2"><p className="text-[var(--admin-text-secondary)]">1. Open authenticator app</p><p className="text-[var(--admin-text-secondary)]">2. Scan QR code</p><p className="text-[var(--admin-text-secondary)]">3. Enter 6-digit code</p></div></div>
